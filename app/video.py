...
def ffmpeg_extract_info(file_path, sz, mt):
    ...
    for p, stream in enumerate(streams):
        ...
        subtitles_list.append(
            f"{title} - [{lang}]" if title else f"Track {p} - [{lang}]"
        )
...
def ffmpeg_get_subs(file, index, legacy, sz, mt):
    if legacy:
        ...
        ass2vtt, codec = False, "ass"

    out = run(
        ["ffmpeg", "-i", file, "-map", f"0:s:{index}", "-f", codec, "-"],
        stdout=PIPE,
        stderr=PIPE,
    )
...
def extract_subtitles(index, file, legacy):
    ...
    if exists(sname) and isfile(sname):
        file = sname
